...
namespace wServer.realm.commands
{
    ...
    internal class RankCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            {
                player.SendInfo("Usage: /rank <PlayerName> <Rank>");
                player.SendInfo("Current ranks: 1, 2, 3, 4, 5");
                return false;
            }
            int MAX = int.Parse(args[1]);
            if (MAX > 5)
            {
                player.SendError("You cannot give ranks above 5.");
                return false;
            }
            player.Manager.Database.DoActionAsync(db =>
            {
                ...
                player.SendInfo("Successfully ranked " + args[0] + " with rank " + args[1]);
                player.UpdateCount++;
                plr.UpdateCount++;
    ...
    internal class RenameCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
                player.SendInfo("Usage: /rename <PlayerName> <Name>");
                return false;
            }
            player.Manager.Database.DoActionAsync(db =>
            {
                var cmd = db.CreateQuery();
                var plr = player.Manager.FindPlayer(args[0]);
                cmd.CommandText = "UPDATE accounts SET name=@name WHERE name=@rename";
                cmd.Parameters.AddWithValue("@name", args[1]);
                cmd.Parameters.AddWithValue("@rename", args[0]);
                player.SendInfo("Successfully renamed " + args[0] + " to " + args[1]);
                plr.SendInfo("You have been renamed to " + args[1] + " by " + player.Name);
                plr.UpdateCount++;
                player.UpdateCount++;
                if (cmd.ExecuteNonQuery() == 0)
                {
